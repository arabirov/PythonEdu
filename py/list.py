...
list('слово')
s = []
l = ['s', 'p', ['isok']]	# создание списка
li = [1, 2912, 213.1, 0, -1, 2]
print(('{1}').format(*l))	# * нужна для работы со списками без явного указания на format
print(('{0[1]}').format(l))	# вариант без *

c = [l * 2 for l in l]	# генерация списка, в конце может использоваться list(l)
print (c, '\n-----------------------------------------------')

c1 = [l * 2 for l in list(l) if l != ['isok']]	# более сложная генерация списка
print (c1, '\n-----------------------------------------------')

c2 = [list(l) + list(d) for l in list(l) if l != 'p' for d in list('word') if d != 'd']	# еще более сложная генерация списка
print(c2, '\n-----------------------------------------------')
